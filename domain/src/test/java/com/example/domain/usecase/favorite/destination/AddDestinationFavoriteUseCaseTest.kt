...
class AddDestinationFavoriteUseCaseTest {

    @Test
    fun givenInvalidDestinationId_whenInvoke_thenReturnsInvalidInputs() = runTest {
        val useCase = AddDestinationFavoriteUseCase(FakeFavoriteDestinationRepository())
    ...
    fun givenRepositorySuccess_whenInvoke_thenReturnsMappedMutationSuccess() = runTest {
        val expected = FavoriteTestFixtures.mutation(isSuccess = true, message = "added")
        val useCase = AddDestinationFavoriteUseCase(
            FakeFavoriteDestinationRepository(
                addResult = Result.Success(expected)
            )
        )
    ...
    fun givenRepositoryFailure_whenInvoke_thenReturnsRepositoryError() = runTest {
        val useCase = AddDestinationFavoriteUseCase(
            FakeFavoriteDestinationRepository(
                addResult = Result.Error(DataError.Network.Timeout)
            )
        )
    ...
    private class FakeFavoriteDestinationRepository(
        private val addResult: Result<FavoriteMutationResult, DataError> = Result.Success(
        ...
        override suspend fun addDestinationToFavorites(
            destinationId: Int
        ): Result<FavoriteMutationResult, DataError> = addResult
